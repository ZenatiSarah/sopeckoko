...
switch (like) {
    ...
    //     }else if (like == 1) {
    //         //Vérifier si le user n'a pas déjà liké ou disliké
    //             Sauce.updateOne({id:req.params.id},{$push : {userLiked: userId}, $inc: {likes : 1}})
    //             .then(res => res.status(200).json("User liked"))
    //             .then(res => res.status(400).json({error}));
            
    //     }else if(like == -1){

    //             Sauce.updateOne({id:req.params.id},{$push : {userDisliked: userId}, $inc: {dislikes : -1}})
    //             .then(res => res.status(200).json("User disliked"))
    //             .then(res => res.status(400).json({error}));
    //     }
    // })
    // .catch((error) => {res.status(404).json({error: error})});
